  ...
  try {
    ...
    if (email && phone && password) {
          ...
          if (testPassword(password)) {
            ...
            if (testPhone(phone)) {
              ...
              if (myResult.rows.length === 0) {
                ...
                await pool.query(
                  "INSERT INTO roles (id, isStudent, isTeacher) VALUES ($1,$2,$3) RETURNING *",
                  [finalResult.rows[0].id, isStudent, isTeacher]
                );
                logger.info(
                  `user ${finalResult.rows[0].firstname} successfully registered`
                );
      ...
      sentry.captureMessage(
        "email or phone or password field can not be empty"
      );
